...
const Footer = () => {
  ...
  return (
    ...
    <footer className="theme-bg-dark border-t border-emerald-400/20 pt-10 pb-4">
      
      {/* --- Main Footer Content Container (Centered Content) --- */}
      <div className="max-w-7xl mx-auto p-4 theme-text-light text-center md:text-left grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-8">
        ...
        <div className="col-span-2 md:col-span-1 mx-auto md:mx-0">
          ...
          <p className="max-w-xs text-sm">
            SikshaHub is the complete platform designed to connect qualified tutors with verified tuition needs, enabling digital tracking and transparent payments.
          </p>
        </div>

        {/* 2. Quick Links - Centered/Aligned */}
        <nav className="flex flex-col gap-2 items-center md:items-start">
          <header className="theme-accent-text font-bold text-lg mb-2">Quick Links</header>
          <Link to="/" className={linkClasses}>Home</Link>
          <Link to="/tuitions" className={linkClasses}>Tuitions</Link>
          <Link to="/tutors" className={linkClasses}>Tutors</Link>
        ...
        <nav className="flex flex-col gap-2 items-center md:items-start">
          <header className="theme-accent-text font-bold text-lg mb-2">Contact</header>
          <a href="mailto:support@sikshahub.com" className={linkClasses}>Email: support@sikshahub.com</a>
          <p>Phone: +880 17XX XXX XXX</p>
          <p>Address: Dhaka, Bangladesh</p>
        </nav>

        {/* 4. Social Media Icons - Centered/Aligned */}
        <nav className="flex flex-col gap-2 items-center md:items-start">
            <header className="theme-accent-text font-bold text-lg mb-2">Connect</header>
            <div className="grid grid-flow-col gap-4">
                {/* X Logo (formerly Twitter) */}
                <a href="#" aria-label="X (formerly Twitter)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className={iconClasses}>
      ...
      <div className="max-w-7xl mx-auto p-4 border-t border-gray-700/50 mt-4 pt-4 theme-text-light text-center">
        <p>Copyright © {new Date().getFullYear()} - All right reserved by {websiteName} LTD.</p>
      </div>
    </footer>
  );
};
